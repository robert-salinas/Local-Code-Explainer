...
app = typer.Typer(
    help="Local Code Explainer - Explica tu código localmente con privacidad."
)
...
def explain(
    ...
    model: str = typer.Option(
        "mistral", "--model", "-m", help="Modelo de Ollama a usar."
    ),
    format: str = typer.Option(
        "cli", "--format", "-f", help="Formato de salida (cli, json)."
    ),
    no_cache: bool = typer.Option(
        False, "--no-cache", help="Desactivar el uso de caché."
    ),
):
    ...
    with typer.progressbar(
        length=100, label="Analizando y generando explicación..."
    ) as progress:
